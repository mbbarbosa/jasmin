(*
*)
Require Import sem_one_varmap sem_one_varmap_facts merge_varmaps2 psem_facts.
Import Utf8.
...
Section LEMMA.
  ...
  Notation write_c := (merge_varmaps2.write_c p extra_free_registers wrf).
  Notation write_I := (merge_varmaps2.write_I p extra_free_registers wrf).
  Notation write_i := (merge_varmaps2.write_i p extra_free_registers wrf).

  Lemma write_c_cons i c :
    Sv.Equal (write_c (i :: c)) (Sv.union (write_I i) (write_c c)).
  Proof. by rewrite /write_c /= merge_varmaps2.write_c_recE. Qed.

  Lemma write_i_if e c1 c2 :
    Sv.Equal (write_i (Cif e c1 c2)) (Sv.union (write_c c1) (write_c c2)).
  Proof.
    rewrite /merge_varmaps2.write_i /merge_varmaps2.write_i_rec /=
            -/(merge_varmaps2.write_c_rec _ _ _ _ c1) -/(merge_varmaps2.write_c_rec _ _ _ _ c2)
            !merge_varmaps2.write_c_recE.
    SvD.fsetdec.
  Qed.
  ...
  Proof.
    ...
    rewrite {1}/write_I merge_varmaps2.write_I_recE -/write_i => dis.
    ...
    by rewrite /write_I merge_varmaps2.write_I_recE -/write_i; clear -hk; SvD.fsetdec.
  Qed.
