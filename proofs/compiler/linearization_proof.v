...
Section HLIPARAMS.
  ...
  Lemma spec_lassign
    ...
    -> truncate_word ws w' = ok w
    -> write_lval [::] x (Vword w) s1 = ok s2
    -> eval_instr lp (lassign liparams ii x ws e) (of_estate s1 fn pc)
       = ok (of_estate s2 fn pc.+1).
  ...
  * whereas internal functions have their frame allocated by the caller *)
  ...
  Let Pfun (ii: instr_info) (k: Sv.t) (s1: estate) (fn: funname) (s2: estate) : Prop :=
    ∀ m1 vm1 body ra lret sp callee_saved,
      ...
      then lsem p' (Lstate m1 vm1 fn 1) (Lstate m2 vm2 caller pc.+1)
  ...
  Lemma lsem_skip_goto m vm fn P m' vm' n ii jj lbl Q R :
    ...
    ~~ has (is_label lbl) Q →
    lsem p' {| lmem := m ; lvm := vm ; lfn := fn ; lpc := size P |} {| lmem := m' ; lvm := vm' ; lfn := fn ; lpc := size P + n.+1 |} →
    lsem p' {| lmem := m ; lvm := vm ; lfn := fn ; lpc := (size P + size Q).+2 |} {| lmem := m' ; lvm := vm' ; lfn := fn ; lpc := size P + n.+1 |}.
  Proof.
  ...
  Proof.
    ...
    { (* expression is the “true” literal *)
      ...
      - (* align; label *)
        apply: (@lsem_step_end _ _ _ p' {| lfn := fn; lpc := size (P ++ add_align ii a [::]) |}); last first.
        + move: C.
  ...
  Proof.
    ...
    { (* second body is empty *)
      ...
      - apply: (@lsem_trans _ _ _ _ {| lmem := m ; lvm := vm ; lfn := fn ; lpc := size (P ++ add_align ii a [::]) |}).
      ...
      { have {ih} := ih fn xH.
       ...
       have X' :
        ...
        + move=> z; have := X z; rewrite /set_RSP -/vrsp.
          ...
          have -> :  {| vtype := sword Uptr; vname := lip_tmp liparams |} = var_tmp by [].
          move=> hnin.
    ...
    ∃ fd,
      ...
      ∀ lm vm args',
        ...
        ∃ vm' lm' res',
          ...
          [/\
            lsem_exportcall p' callee_saved lm fn vm lm' vm',
            match_mem m' lm',
